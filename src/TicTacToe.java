...
public class TicTacToe extends UnicastRemoteObject implements ITicTacToe {
    ...
    // Removes player and match objects whose timeout has expired
    public void garbageCollector() {
        ...
        try {
            while (true) {
                // Creates a list of players to be removed
                ...
                // Creates a list of matches to be removed
                ...
                try {
                    for (Match m : matches) {
                        if (m.hasTimedOut()) {
                            System.out.println("[Garbage Collector] Removing match!");
                            matchesDelete.add(m);
                        }
                    }
                } finally {
            ...
            System.err.println("Server garbage collector failure!");
        }
    }

    // Registers players on the server
    public int addPlayer(String name) throws RemoteException {
        ...
        try {
            ...
            System.out.println("User " + p.getName() + " (" + p.getId() + ") joined!");
            p.updateTimestamp();
            return p.getId();
        } finally {
            id_writeLock.unlock();
...
public int hasMatch(int id) throws RemoteException {
    ...
    // Checks if there is any match waiting for a second player
    matches_writeLock.lock();
    try {
        for (Match m : matches) {
            if (!m.isReady()) {
    ...
    // Otherwise, create a new match waiting for a second player
    matches_writeLock.lock();
    try {
        Match m = new Match(p);
        matches.add(m);
...
public int isMyTurn(int id) throws RemoteException {
    ...
    Player opponent = (p == m.getPlayer1()) ? m.getPlayer2() : m.getPlayer1();

    if (opponent == null && m.isReady()) {
        return 5;
    }

    if (opponent.hasTimedOut()) {
        return 5;
    }
    ...
    Player winner = p.getMatch().getWinner();
    if (winner != null) {
        if (winner == p) {
            return 2;
        } else {
    ...
    if (p.getMatch().getCurrentPlayer() == p) {
        ...
        return 0; // It's the opponent's turn
    }
}
...
public int move(int id, int row, int column) throws RemoteException {
    ...
    if (m.getCurrentPlayer() != p) {
        return -4; // Not the player's turn
    }
